...
for inFile in inFile_Array:
        ...
        infile_label = inFile.split('_')[-1].split('.')[0]
        infile_labels.append(infile_label)
#### to be optimized ####
...
for inFile in inFile_Array:
    ...
    for case in cases[1:]:
        ...
        ####################################################################
        P_coupled = P_nuclear + P_windfarm + P_pv
        ...
        if '10' in num_chars: 
            ...
            P_h2_produced = [0.0] * len(time)
            P_h2_consumed = [0.0] * len(time)
            P_abandon = []
            m_stored_data = [0.0] * len(time)
            for i in range(len(time)):
                P_excess = P_coupled[i] - P_demand[i]
                if P_excess > 0.0:
                    P_abandon.append(P_excess)
                else:
                    P_abandon.append(0.0)
